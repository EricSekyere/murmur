...
| `wake`     | ort        | No      | Wake-word detection ("Hey Murmur"); shares the ort runtime with `vad` |
...
For user-visible changes, add a `Whats-New: Title | one-sentence body` trailer to the commit — the release build turns these into the in-app What's New dialog bullets (see `scripts/release.sh`). A commit may carry several such trailers; without one, feat/perf subjects are used as plain titles, and fixes don't appear at all. `Whats-New: skip` keeps a commit's own subject out of the dialog; `Whats-New-Skip: <7+ hash chars>` on any later commit retracts everything an already-pushed commit contributed (dialog only — GitHub notes and version bump are unaffected).
