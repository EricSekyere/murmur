...
from wake_word_training.scoring import score_windows
...
def test_score_windows_feeds_the_streaming_batch_of_one():
    ...
    assert score_windows(session, windows) == [0.5, 0.5, 0.5]
    assert session.seen == [(1, 16, 96)] * 3


def test_score_windows_handles_empty_input():
    assert score_windows(_Batch1Session(), np.zeros((0, 16, 96), np.float32)) == []
